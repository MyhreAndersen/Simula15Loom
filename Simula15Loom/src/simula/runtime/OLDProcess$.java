 ...
 */
public class OLDProcess$ extends Link$ {
    public boolean isDetachUsed() { return(true); }

	public OLDEVENT_NOTICE$ EVENT = null;

	// Constructor
	public OLDProcess$(final RTObject$ staticLink) {
		super(staticLink);
	}
	
	public OLDProcess$ STM$() {
		detach();
		// INNER
		// TERMINATED$ = true;
		terminate();
	...
	protected void terminate() {
		OLDProcess$ nxtcur = ((OLDSimulation$) SL$).passivate1();
		resume(nxtcur,false); // Special Case without Swap
		// Signal special action in RTObject$.EBLK
		OLDProcess$.this.STATE$=OperationalState.terminatingProcess;
	}
	...
	public OLDProcess$ nextev() {
		if (idle())	return (null);
		OLDEVENT_NOTICE$ suc=((OLDSimulation$) SL$).SQS.nextAfter(EVENT);
		if (suc == null)
			return (null);
		return (suc.PROC);
