 ...
 */
public class OLDSimulation$ extends Simset$ {
	...
	// Constructor
	public OLDSimulation$(RTObject$ staticLink) {
		super(staticLink);
		SQS=new SequencingSet$();
		main = (MAIN_PROGRAM$) new MAIN_PROGRAM$((OLDSimulation$) CUR$).START$();
		main.EVENT = new OLDEVENT_NOTICE$(0, main);
		SQS.add(main.EVENT,false);
	}

	public OLDSimulation$ STM$() {
...
//		main.EVENT.into(OLD_SQS);
	 ...
	 * </pre>
	 */
	public OLDProcess$ current() {
		return (SQS.first().PROC);			
	}

	public void hold(final double T) {
		SIM_TRACE("Hold " + T);
		OLDEVENT_NOTICE$ first = SQS.pollFirst();
		if (first != null) {
			if (T > 0) first.EVTIME = first.EVTIME + T;
			OLDEVENT_NOTICE$ suc = SQS.first();
			SQS.add(first,false);
			if (suc != null) {
	...
	public void cancel(final OLDProcess$ x) {
		SIM_TRACE("Cancel " + x);
	...
	public void ActivateDirect(final boolean REAC,final OLDProcess$ X) {
		if (X == null)
			...
			OLDProcess$ z;
			OLDEVENT_NOTICE$ EV = null;
			if (REAC) EV = X.EVENT;
			else if (X.EVENT != null) return;
			z = current();
			X.EVENT = new OLDEVENT_NOTICE$(time(), X);
	...
	public void ActivateDelay(final boolean REAC,final OLDProcess$ X,final double T,final boolean PRIO) {
		ActivateAt(REAC, X, time() + T, PRIO);
	}

	public void ActivateAt(final boolean REAC,final OLDProcess$ X,double T,final boolean PRIO) {
		if (X == null)
			TRACE_ACTIVATE(REAC, "none");
		else if (X.STATE$ == OperationalState.terminated)
			TRACE_ACTIVATE(REAC, "terminated process");
			...
			z = current();
			if (T < time())	T = time();
			X.EVENT = new OLDEVENT_NOTICE$(T, X);
	...
	public void ActivateBefore(final boolean REAC,final OLDProcess$ X,final OLDProcess$ Y) {
		ACTIVATE3(REAC, X, true, Y);
	}

	public void ActivateAfter(final boolean REAC,final OLDProcess$ X,final OLDProcess$ Y) {
		ACTIVATE3(REAC, X, false, Y);
	}

	private void ACTIVATE3(final boolean REAC,final OLDProcess$ X,final boolean BEFORE,final OLDProcess$ Y) {
		if (X == null)
			TRACE_ACTIVATE(REAC, " none");
		else if (X.STATE$ == OperationalState.terminated)
			TRACE_ACTIVATE(REAC, " terminated process");
			...
			if (Y == null || Y.EVENT == null) X.EVENT = null;
				...
				X.EVENT = new OLDEVENT_NOTICE$(EVTIME, X);
